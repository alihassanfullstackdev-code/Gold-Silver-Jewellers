...
import CheckoutModal from './CheckoutModal'; // Import Referenced Modal
...
export default function CartDrawer({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  // Reusable Checkout logic mapped to Backend Controller keys
  const handleFinalCheckout = async (customerDetails: any) => {
    ...
    try {
      ...
      console.error("Checkout Drawer Error:", error);
              ...
              {cartItems.length > 0 && (
                <div className="p-8 bg-black/60 border-t border-white/5 space-y-6 backdrop-blur-lg">
                  ...
                  <button
                    ...
                    className="w-full py-5 bg-gold text-black font-black text-[10px] uppercase tracking-[0.5em] hover:bg-white transition-all duration-700 flex justify-center items-center gap-3"
                  >
                    <Truck size={16} />
                    Place COD Order
                  </button>
